...
//
// version on 2026-08-19 - stripped back down to shake magnitude only,
// dropping direction/angle_deg entirely (was: raw atan2, then a full
// gyro+accel+magnetometer Madgwick/complementary-filter orientation
// stack - see git history if reviving any of that). Real-hardware
// feedback across several sessions never got direction sensing on a
// handheld stick reliably working ("cannot seem to find a set coordinate
// system"), and the LED effect side made it unnecessary anyway:
// ShakeFireworkEffect (src/output/effects/led_effects.py) replaced the
// old direction-mapped TriArmGlideEffect with a vigorous-shake-fires-
// every-arm-at-once design that only ever needed shake magnitude. Simpler
// firmware, one less whole class of bug, nothing lost that's actually
// used downstream.
...
// Reads the IMU and returns the current shake magnitude, clamped to [0, 1].
...
// favour of an idle invite instead: "SHAKE ME" while the stick's resting,
...
// of flapping. Untuned placeholders like every other threshold in this
// file (see class-level SENSITIVITY comment) - raise SWING_INVITE_HIDE_ABOVE
// further if ordinary handling still dismisses it too easily on real
// hardware.
